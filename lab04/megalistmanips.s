...
mapLoop:
    ...
    # fix error, should save the temporary
    addi sp, sp, -12
    sw t0, 0(sp)
    sw t1, 4(sp)
    sw t2, 8(sp)
    ...
    # fix error, shoud restore the temporary
    lw t0, 0(sp)
    lw t1, 4(sp)
    lw t2, 8(sp)
    addi sp, sp, 12
    ...
    # fix error, offset should be 4
    addi t1, t1, 4

    bne t0, t2, mapLoop # repeat if we haven't reached the array size yet

    # fix error, should load word
    lw a0, 8(s0)        # load the address of the next node into a0

    # fix error, should just assign
